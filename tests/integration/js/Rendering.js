...
Renderer.prototype =
{
	...
	_begin_drawing: function(pivot, position, rotation, style)
	{
		...
		if (style.fill === FillStyle.Wireframe)
		{
			this._context.strokeStyle = style.color;
			this._context.lineWidth = style.line_width;
		}
		else if (style.fill === FillStyle.Solid)
		{
...
function Animation(on_begin, on_frame, on_pause)
{
	this._on_start = on_begin;
...
Animation.prototype =
{
	...
	{
		this._frame_request = requestAnimationFrame(() => this.step());
		if (this._on_frame) { this._on_frame(this); }
	},
